...
void generateAccountPin(FILE *fPtr);

int ensureFileInitialized(FILE *fPtr);
int ensurePinFileInitialized(void);
int readPin(unsigned int accountNum, unsigned int *pin);
int writePin(unsigned int accountNum, unsigned int pin);
unsigned int generateRandomPin(void);
...
int ensurePinFileInitialized(void)
{
    ...
    while (currentSize < expectedSize)
    {
        if (fwrite(&blankPin, sizeof(unsigned int), 1, pinPtr) != 1)
        {
            fclose(pinPtr);
            return 0;
        }
...
int readPin(unsigned int accountNum, unsigned int *pin)
{
    ...
    if (fread(pin, sizeof(unsigned int), 1, pinPtr) != 1)
    {
        fclose(pinPtr);
        return 0;
    }
...
int writePin(unsigned int accountNum, unsigned int pin)
{
    ...
    if (fwrite(&pin, sizeof(unsigned int), 1, pinPtr) != 1)
    {
        fclose(pinPtr);
        return 0;
    }
...
unsigned int generateRandomPin(void)
{
    return (unsigned int)(MIN_PIN + (rand() % (MAX_PIN - MIN_PIN + 1)));
}
...
int main(int argc, char *argv[])
{
    ...
    {
        puts("Unable to initialize PIN storage.");
        fclose(cfPtr);
        return 1;
    }
...
void deleteRecord(FILE *fPtr)
{
    ...
    if (!writePin(accountNum, 0))
    {
        puts("Warning: PIN reset failed.");
    }
...
void newRecord(FILE *fPtr)
{
    ...
    if (!writePin(accountNum, 0))
    {
        puts("Warning: PIN storage update failed.");
    }
...
void generateAccountPin(FILE *fPtr)
{
    unsigned int accountNum;
    unsigned int pin;
    unsigned int existingPin = 0;
    char details[180];
    struct clientData client = {0, "", "", 0.0};

    printScreenHeader("GENERATE ACCOUNT PIN");

    if (!promptUnsignedInRange("Enter account number ( 1 - 100 ): ", 1, MAX_ACCOUNTS, &accountNum))
    {
        return;
    }
    ...
    if (readPin(accountNum, &existingPin) && existingPin != 0)
    {
        printf("Existing PIN for account %u: %04u\n", accountNum, existingPin);
    }

    pin = generateRandomPin();
    if (!writePin(accountNum, pin))
    {
        puts("Failed to store generated PIN.");
        waitForEnter();
        return;
    }

    printMessageBox("SUCCESS", "PIN generated successfully.");
    printf("Account %u new PIN: %04u\n", accountNum, pin);
    snprintf(details, sizeof(details), "PIN generated for account %u", accountNum);
    logTransaction("PIN", details);
    waitForEnter();
}
...
unsigned int enterChoice(void)
{
    ...
    while (1)
    {
        ...
        if (strcmp(menuChoice, "7") == 0 || strcmp(menuChoice, "p") == 0 || strcmp(menuChoice, "pin") == 0)
        {
            return 7;
        }
        if (strcmp(menuChoice, "8") == 0 || strcmp(menuChoice, "x") == 0 || strcmp(menuChoice, "exit") == 0)
        {
            return 8;
        }

        puts("Invalid choice. Use 1-8, E/U/A/D/L/T/P/X, or full commands.");
    }
}
